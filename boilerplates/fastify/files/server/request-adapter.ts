...
// Inspired by https://github.com/honojs/node-server/blob/main/src/request.ts
...
export const newRequest = (incoming: IncomingMessage | Http2ServerRequest, defaultHostname?: string) => {
  ...
  const url = new URL(
    `${
      incoming instanceof Http2ServerRequest || (incoming.socket && (incoming.socket as TLSSocket).encrypted)
        ? "https"
        : "http"
    }://${host}${incoming.url}`,
  );
