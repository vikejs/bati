...
const SentrySourcemapsPlugin = (process.env?.SENTRY_DSN ?require('./esbuild-sentry-plugin.cjs'):undefined)


const esbuildPlugins = (/** @type {any} */ serverless) => {
  ...
  return [
    nativeNodeModules, copyPlugin,
    ...(SentrySourcemapsPlugin ? [SentrySourcemapsPlugin(serverless)] : [])
  ]
}
