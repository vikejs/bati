...
export default async function getEnv(props: TransformerProps) {
  ...
  if (!auth0ClientId || !auth0ClientSecret || !auth0BaseUrl) {
    if (process.env.CI) {
      throw new Error("Must define TEST_AUTH0_CLIENT_ID, TEST_AUTH0_CLIENT_SECRET and TEST_AUTH0_ISSUER_BASE_URL");
    }
    console.warn("No TEST_AUTH0_CLIENT_ID, TEST_AUTH0_CLIENT_SECRET or TEST_AUTH0_ISSUER_BASE_URL defined");
  }
  ...
  envContent = appendToEnv(envContent, "CLIENT_SECRET", auth0ClientSecret ?? "", "Auth0 Client Secret");
