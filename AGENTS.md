...
- `skills(meta) => BatiSkill[]` - **agent skills** (see `SKILLS_PLAN.md`): SKILL.md skills this feature contributes (`name`, `description`, `body`, optional `allowedTools`). Composed centrally via `packages/core/src/skills.ts` into `.agents/skills` / `.claude/skills` (plus a generated `AGENTS.md`) only when an AI agent is selected. See `boilerplates/hono/bati.config.ts`.
