...
async function main(context: GlobalContext, args: mri.Argv<CliOptions>) {
  ...
  // CI provides a prebuilt batijs-tests-utils tarball as an artifact; locally
  // we produce one once and reuse it across every dokploy combination so the
  // sibling .e2e/ workspaces can install via a stable file: dependency.
    ...
    promises.push(
      limit(async () => {
        ...
        if (flags.includes("dokploy")) {
          // Dokploy: create a sibling host-only workspace and leave the app dir untouched.
          // The sibling lives next to <projectDir> so pnpm/bun pick it up via packages/*.
          // The packed tarball lives in <e2eDir> (never in the app dir) so the docker
          // build context stays free of test artefacts.
          const e2eDir = `${projectDir}.e2e`;
          await mkdir(e2eDir);
          let testsUtilsRef = "link:@batijs/tests-utils";
          if (packedTestsUtilsTgzPath) {
          ...
          await Promise.all([
            ...
            createE2EWorkspace(e2eDir, basename(projectDir), flags, testsUtilsRef),
          ]);
        } else {
          // Non-dokploy: mutate the app dir in place (existing behavior).
          await updatePackageJson(projectDir, flags);
          await Promise.all([
            ...testFiles.map((f) => copyFile(f, join(projectDir, basename(f)))),
