...
// TODO: remove this function and move the code inside validateKnownSetupFeatures()
function hasAdditionalSetupSteps(flags: string[]): boolean {
  const hasTodo = [
    'auth0', 'aws', 'd1', 'drizzle', 'mantine', 'prisma', 'sentry', 'shadcn-ui', 'sqlite'
  ];

  return flags.some(flag => hasTodo.includes(flag));
}
...
function validateKnownSetupFeatures(flags: string[], knownSetupFeatures: string[], projectPath: string): void {
  const noTodo = ['react', 'vue', 'solid'];

  const readmePath = join(projectPath, 'README.md');

  // Assert that the README.md file exists - it should have been generated by this point
  assert(existsSync(readmePath), `README.md should exist at ${readmePath} after project generation`);
...
function printOK(dist: string, flags: string[]): void {
  ...
  // TODO: move most new logic outside this function. Keep it simple.
  // TODO: Always assert, not only in production
  if (process.env.NODE_ENV !== 'production') {
    assert(typeof dist === 'string' && dist.length > 0, 'project path should be a non-empty string');
...
// TODO: remove message option
function assert(condition: boolean, message: string): asserts condition {
  if (!condition) {
    throw new Error(`You sumbled upon an inernal bug in the scaffolder. Reach out on GitHub.`);
  }
}
