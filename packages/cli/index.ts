...
/**
 * Simple assertion function for development-time checks
 */
...
function hasAdditionalSetupSteps(flags: string[]): boolean {
  ...
  // Assert our assumptions about the setup
  assert(knownSetupFeatures.length > 0, 'knownSetupFeatures should not be empty');
  assert(Array.isArray(flags), 'flags should be an array');
...
function validateKnownSetupFeatures(flags: string[], knownSetupFeatures: string[], projectPath: string): void {
  ...
  try {
    ...
    // If we can't read the file, that's unexpected since we asserted it exists
    assert(false, `Failed to read README.md at ${readmePath}: ${error}`);
  }
}
