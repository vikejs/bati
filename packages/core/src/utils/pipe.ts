// Extracted form https://github.com/MathisBullinger/froebel/blob/4860d718c8806a0bba026a2b955c2fb5b478359e/pipe.ts
 ...
 * ```
 */
const pipe = <T extends [λ, ...λ[]]>(...funs: PipeReturn<T> extends never ? never : T) =>
  ((...args) => {
...
type CheckPipe<F extends λ[], D extends FunDef[], Async extends boolean = false> = F extends [any, any, ...any[]]
  ? Resolved<D[0][1]> extends Parameters<F[0]>
    ? CheckPipe<
        F extends [any, ...infer F_] ? (F_ extends λ[] ? F_ : never) : never,
        D extends [any, ...infer D_] ? (D_ extends FunDef[] ? D_ : never) : never,
